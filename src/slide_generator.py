...
# ---------------------------------------------------------------------------
# Brand palette (from HBS Africa Business Club / Owning Tomorrow slide)
# ---------------------------------------------------------------------------
NAVY  = RGBColor(0x1B, 0x2A, 0x4A)
GREEN = RGBColor(0x9D, 0xC5, 0x28)
AMBER = RGBColor(0xC4, 0x73, 0x29)
...
def _solid_bg(slide, color: RGBColor):
    """Fill the entire slide with a solid colour (placed at z=0)."""
...
def _set_alpha(shape, opacity_pct: int):
    """
    Set the fill opacity of a shape.
...
def _add_bg_image(slide, image_path: str):
    """Add a preprocessed square image as the slide background."""
...
def _add_overlay(slide, opacity_pct: int = 65):
    """Overlay a semi-transparent dark-navy rectangle over the background."""
    rect = slide.shapes.add_shape(1, 0, 0, SLIDE_W, SLIDE_H)
    rect.fill.solid()
...
def _add_bottom_bar(slide, label: str = "HBS Africa Business Club"):
    bar = slide.shapes.add_shape(
        1, 0, SLIDE_H - BOTTOM_BAR_H, SLIDE_W, BOTTOM_BAR_H
    )
    bar.fill.solid()
...
def preprocess_image(src_path: str, out_path: str, size: int = 2160) -> str:
    """
    Centre-crop to square and resize.  Returns out_path.
    Saves as JPEG for smaller PPTX file size.
    """
...
def _title_slide(prs, bg_path, panel_name, event_name, event_date):
    ...
    # Club label
    _textbox(
        slide, "HBS AFRICA BUSINESS CLUB",
    ...
    # Panel name
    _textbox(
        slide, panel_name or "Panel Discussion",
        MARGIN, Inches(1.85), CONTENT_W, Inches(3.2),
        size=36, color=WHITE, bold=True,
    )

    # Event + date
    bottom_text = event_name
    if event_date:
        bottom_text += f"  ·  {event_date}"
    _textbox(
        slide, bottom_text,
...
def _takeaway_slide(prs, takeaway: dict, bg_path: str, event_name: str):
    ...
    num_tb = slide.shapes.add_textbox(
        MARGIN, Inches(0.65), Inches(0.55), Inches(0.55)
    )
...
def _quote_slide(prs, quote: dict, event_name: str):
    ...
    # Top green bar (taller for quotes)
...
def generate_pptx(
    ...
    """
    Generate the full PPTX and save to output_path.

    Args:
        analysis:    dict from analyzer.analyze_transcript()
        bg_images:   list of uploaded image paths (1-5 files)
        output_path: where to write the .pptx file
        panel_name:  display name of the panel
        event_name:  conference name shown in branding bar
        event_date:  optional date string
        work_dir:    temp directory for pre-processed images
    ...
    # Pre-process background images (square crop + resize)
    processed = []
